...
def find_weights(random_nums, iterations, sigma=1):
    ...
    return np.dot(A, np.array([np.sin(x) for x in random_nums], dtype=np.float64).T)


def find_result(x, weights, random_nums, sigma=1):
    
    approx = np.float64(np.dot(np.array([find_fi(x,y,sigma) for y in random_nums], dtype=np.float64), weights.T))

    print(f'Estimated value of sin({x}) = {approx}')
    print(f'Actual value of sin({x}) = {np.sin(x)}', end='\n\n')
    print(f'Error = {np.abs(np.sin(x)-approx)}')
...
def main():
    ...
    parser.add_argument("-s", "--sigma", nargs='?', help="σ parameter value", const=1, type=float, default=1)
    parser.add_argument("iterations", type=int, help="specify number of instances to train the network")
    ...
    find_result(np.float64(args.num), weights, random_nums, sigma=args.sigma)
